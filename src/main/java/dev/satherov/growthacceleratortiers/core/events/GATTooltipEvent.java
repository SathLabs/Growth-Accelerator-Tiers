...
public class GATTooltipEvent {
    ...
    public static void registerTooltips(ItemTooltipEvent event) {
        ...
        if (event.getItemStack().is(GATBlocks.CRANKED_GROWTH_ACCELERATOR.asItem())) {
            ...
            event.getToolTip().add(Component.translatable("tooltip.growthacceleratortiers.directional_growth_accelerator").withStyle(ChatFormatting.GRAY));
            event.getToolTip().add(Component.translatable("tooltip.growthacceleratortiers.change_direction").withStyle(ChatFormatting.GRAY));
        }
    }
}
