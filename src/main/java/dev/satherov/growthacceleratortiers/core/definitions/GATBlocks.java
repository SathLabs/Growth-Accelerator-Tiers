...
public class GATBlocks {
    ...
    private static <T extends Block> BlockDefinition<T> block(String englishName, Identifier id, Function<BlockBehaviour.Properties, T> blockSupplier) {
        return GATBlocks.block(englishName, id, blockSupplier, null);
    }
    
    private static <T extends Block> BlockDefinition<T> block(String englishName, Identifier id, Function<BlockBehaviour.Properties, T> blockSupplier, @Nullable BiFunction<Block, Item.Properties, BlockItem> itemFactory) {
        Preconditions.checkArgument(id.getNamespace().equals(GAT.MOD_ID));
        
        // Create block and matching item
        var deferredBlock = GATBlocks.DR.registerBlock(id.getPath(), blockSupplier);
